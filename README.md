...
また今回はCSVでの分析ですが、ダッシュボードの設計自体は Snowflake等のDWHとの接続を想定しています。

ｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰｰ

9.追加分析：Tableauによる可視化
Pythonでの集計結果をベースに、Tableauを用いて「次に打つべき施策」を意思決定するためのダッシュボードを作成しました。

■ダッシュボード
[https://public.tableau.com/shared/ZJRYDQB3W?:display_count=n&:origin=viz_share_link]

■ ビジネスアクションに繋げるための判定ロジック
本分析では、「リソース（予算・時間）をどこに集中すべきか」を判断するためのロジックを実装しました。

・High-Potential (Focus): Rating ≥ 4.5 AND Review < 300
狙い: 「顧客満足度は極めて高いが、まだ世に見つかっていない商品」を特定します。これらは広告投下や露出増により、一気に成長する可能性を秘めたグループです。

・Market Established: Review ≥ 1000
狙い: すでに市場の信頼を獲得している主力商品です。過度な広告は不要ですが、欠品によるチャンスロスを防ぐための在庫管理や、リピート施策に注力すべきグループと定義します。

Standard: その他
狙い: レビュー内容を分析し、商品力や価格の再設計を検討すべきグループ。

■ 工夫した点
・「高評価だが埋もれている商品」を特定し、投資対効果を最大化させる視点の導入
・ラベルの整列など、視認性を高めた微細なデザイン調整
